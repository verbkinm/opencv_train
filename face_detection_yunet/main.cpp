        ...
        while (1)
        {
            ...
            case 'p': //tap 'P' to save.
                if(faces.rows == 1)
                {
                    ...
                    cv::Mat saveImg = frame(cv::Rect(x, y, w, h));
                    resize(saveImg, saveImg, cv::Size(128, 128));
                    cvtColor(saveImg, saveImg, cv::COLOR_BGR2GRAY);

                    static int  faceseq = 0;
                    std::string path = "Img/1";
                    std:: string strname = cv::format("%s/%d.pgm", path.c_str(), faceseq++);
                    if(imwrite(strname, saveImg) == false)
                    {
                        printf("Image write failed!\n");
                        printf("Please check filename[%s] is legal!\n", strname.c_str());
                        return -1;
                    }
                    imshow(strname, saveImg);
                }
                break;
